...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    trafficstarCampaigns = pgTable("trafficstar_campaigns", {
      id: serial("id").primaryKey(),
      trafficstarId: text("trafficstar_id").notNull(),
      ...
      async clearCampaignLogs(campaignId) {
        try {
          ...
          `;
              ...
              const result = await pool2.query(query, [campaignId]);
...
`;
      ...
      async clearAllLogs() {
        try {
          const files = fs5.readdirSync(this.logDirectory);
          ...
          for (const file of campaignLogFiles) {
            const filePath = path6.join(this.logDirectory, file);
          ...
          console.log(`\u{1F9F9} Cleared all URL budget logs - ${campaignLogFiles.length} files emptied`);
...
async function processTrafficGenerator(campaignId, forceMode) {
  try {
    ...
    if (forceMode === "force_activate") {
      console.log(`\u{1F4AA} FORCE MODE: Forcing activation of campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
      try {
        const today = /* @__PURE__ */ new Date();
        const todayStr = today.toISOString().split("T")[0];
        const endTimeStr = `${todayStr} 23:59:00`;
      ...
      console.log(`\u{1F4AA} FORCE MODE: Forcing pause of campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
      try {
        await pauseTrafficStarCampaign(campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
      ...
      if (activeUrls.length === 0) {
        ...
        if (statusMatches(currentStatus, "active")) {
          ...
          try {
            ...
            console.log(`\u{1F504} Starting minute-by-minute EMPTY URL status check for campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
          } catch (error) {
        ...
        if (statusMatches(status, "paused")) {
          ...
          try {
            ...
            startMinutelyStatusCheck(campaign.id, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
          } catch (error) {
  ...
  try {
    ...
    if (forceMode === "force_activate") {
      ...
      try {
        ...
        await trafficStarService.activateCampaign(Number(campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id));
        console.log(`\u2705 FORCE MODE: Successfully activated campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
        startMinutelyStatusCheck2(campaignId, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
        return;
      } catch (error) {
        console.error(`\u274C FORCE MODE: Error activating campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}:`, error);
        return;
      ...
      console.log(`\u{1F4AA} FORCE MODE: Forcing pause of campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
      try {
        await pauseTrafficStarCampaign2(campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
        console.log(`\u2705 FORCE MODE: Successfully paused campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
        startMinutelyPauseStatusCheck2(campaignId, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
        return;
      } catch (error) {
        console.error(`\u274C FORCE MODE: Error pausing campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}:`, error);
        return;
    ...
    const spentValue = await getTrafficStarCampaignSpentValue2(campaignId, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
    if (spentValue === null) {
      console.error(`Failed to get spent value for campaign ${campaignId}`);
      return;
    }
    console.log(`Campaign ${campaignId} spent value: $${spentValue.toFixed(4)}`);
    await handleCampaignBySpentValue2(campaignId, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id, spentValue);
  } catch (error) {
    console.error(`Error processing Traffic Generator for campaign ${campaignId}:`, error);
  }
}
...
async function pauseTrafficStarForEmptyCampaigns() {
  try {
    ...
    for (const campaign of campaignsWithTrafficStar) {
      ...
      if (activeUrls.length === 0) {
        console.log(`Campaign ${campaign.id} has NO active URLs - will pause TrafficStar campaign ${campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id}`);
        const currentStatus = await getTrafficStarCampaignStatus2(campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
        if (currentStatus === "active") {
...
async function debugProcessCampaign2(campaignId) {
  try {
    ...
    const spentValue = await getTrafficStarCampaignSpentValue2(campaignId, campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
    const status = await getTrafficStarCampaignStatus2(campaign.trafficstarCampaignId || campaign.trafficstar_campaign_id);
    let totalRemainingClicks = 0;
    let activeUrls = 0;
    let inactiveUrls = 0;
    let urlDetails = [];
...
var DatabaseStorage = class {
  ...
  async fullSystemCleanup() {
    try {
      ...
      try {
        await db.execute(sql2`DELETE FROM gmail_campaign_assignments`);
